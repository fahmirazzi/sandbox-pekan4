...
import { Ayat, BookmarkAyat, SurahDetail } from "@/app/interface";
...
export default function SurahDetailPage() {
  ...
  const [audioPlay, setAudioPlay] = useState<number | null>(null);
  const [bookmarks, setBookmarks] = useState<BookmarkAyat[]>([]);
  const audioRef = useRef<HTMLAudioElement | null>(null);

  useEffect(() => {
    const localStorageData = localStorage.getItem("quran-bookmarks");
    if (localStorageData) {
  ...
  const handleToggleBookmark = (ayatBookmarked: BookmarkAyat) => {
    setBookmarks((prevBookmarks) => {
      const isAlreadyBookmarked = prevBookmarks.some(
        (bm) =>
          bm.nomorSurah === ayatBookmarked.nomorSurah &&
          bm.ayat === ayatBookmarked.ayat
      );
        ...
        <div className="space-y-6">
          {data.ayat.map((ayat) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors ${
                    audioPlay === ayat.nomorAyat
                      ? "bg-green-600 hover:bg-green-700 text-white"
                      : "bg-slate-700 hover:bg-slate-600 text-slate-300"
                  }`}
                  title={
                    audioPlay === ayat.nomorAyat ? "Stop Audio" : "Play Audio"
                  }
                >
                  {audioPlay === ayat.nomorAyat ? (
                    <svg
                  ...
                  <span className="text-sm">
                    {audioPlay === ayat.nomorAyat ? "Stop" : "Play"}
                  </span>
                </button>
              </div>
              ...
              <button
                onClick={() =>
                  handleToggleBookmark({ nomorSurah, ayat: ayat.nomorAyat })
                }
                className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors ${
                  bookmarks.some(
                    (bm) =>
                      bm.nomorSurah === nomorSurah && bm.ayat === ayat.nomorAyat
                  )
                    ? "bg-green-600 hover:bg-green-700 text-white"
                    : "bg-slate-700 hover:bg-slate-600 text-slate-300"
                }`}
